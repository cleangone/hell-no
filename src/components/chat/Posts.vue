<template>
   <div v-if="viewMgr.isXs" class="w-100">
      <Post v-for="post in posts" :key="post.id" :post="post" :chat="chat" @popup="onPopup" />          
   </div>
...
<script setup>
   ...
   const posts = computed(() => props.chat ? toSortedDateCreatedAsc(postStore.getPosts(props.chat.id)) : [])
